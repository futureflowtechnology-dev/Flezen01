...
HELP_TEXT = (
    ...
    "  dedicated queue instead of the shared broadcast queue\n"
    "/toggledestshuffle <name> — shuffle order for that destination's queue\n\n"
    "SENDING\n"
    "/sendnow — send everything immediately (active + all dedicated queues)\n"
    ...
    "/toggleshuffle — random vs in-order posting from the shared queue\n"
...
async def add_destination(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat["destinations"][name] = {
        ...
        "queue": None, "shuffle": False,
    }
    await storage.save()
    await update.message.reply_text(
        f"✅ Destination '{name}' -> {dest_id} added (broadcast mode — shares the "
        f"active queue). Use /setdestqueue {name} <queue> to give it its own "
        f"independent, optionally shuffled queue."
    )
...
async def list_destinations(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for name, d in chat["destinations"].items():
        ...
        qname = d.get("queue")
        queue_desc = f"dedicated:'{qname}'" if qname else f"broadcast:'{chat['active_queue']}'"
        shuffle_desc = "🔀shuffle" if d.get("shuffle") else "➡️in-order"
        lines.append(
            f"• {name} ({d['chat_id']}) — {state} {health}  sent:{d['sent']} failed:{d['failed']}\n"
            f"   queue: {queue_desc}, {shuffle_desc}"
        )
        buttons.append([InlineKeyboardButton(
...
async def set_dest_queue(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    chat = await storage.get_chat(chat_id)

    if len(context.args) < 2:
        await update.message.reply_text(
            "❌ Usage: /setdestqueue <destination_name> <queue_name|none>\n"
            "Assigns a destination its own independent (optionally shuffled) "
            "queue, instead of sharing the active/broadcast queue. Use "
            "'none' to put it back on the shared broadcast queue."
        )
        return
    ...
    qname = context.args[1]
    if qname.lower() == "none":
        dest["queue"] = None
        msg = f"✅ '{name}' now shares the broadcast queue '{chat['active_queue']}'."
    else:
        chat["queues"].setdefault(qname, [])
        dest["queue"] = qname
        msg = (
            f"✅ '{name}' now pulls from its own dedicated queue '{qname}' "
            f"({len(chat['queues'][qname])} items), independent of other destinations."
        )
    await storage.save()
    await update.message.reply_text(msg)


async def toggle_dest_shuffle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    chat = await storage.get_chat(chat_id)

    if not context.args:
        await update.message.reply_text("❌ Usage: /toggledestshuffle <destination_name>")
        return
    ...
    dest["shuffle"] = not dest.get("shuffle", False)
    await storage.save()
    state = "ON" if dest["shuffle"] else "OFF"
    queue_desc = dest.get("queue") or f"broadcast:'{chat['active_queue']}'"
    await update.message.reply_text(
        f"🔀 Shuffle for destination '{name}' is now {state} (queue: {queue_desc})."
    )
...
async def send_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    chat = await storage.get_chat(chat_id)
    total = scheduler._remaining_total(chat)
    ...
    await update.message.reply_text(
        f"🚀 Sending {total} item(s) now (ignores interval; includes any "
        f"dedicated per-destination queues)..."
    )
    ...
    await utils.notify_admin(context.bot, f"✅ Chat {chat_id} sent {sent} via /sendnow.")
...
async def set_last_word_replacement(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "(Reminder: this only applies once /togglelaststrip is ON.)"
    )
...
async def set_tg_link_replacement(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "(Reminder: this only applies once /toggletglinkreplace is ON.)"
    )
...
async def preview_caption(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    settings = chat["settings"]
    original_caption = queue[0].get("caption")
...
async def show_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"Shuffle broadcast queue: {'ON' if s.get('shuffle') else 'OFF'}\n"
...
async def toggle_shuffle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🔀 Shuffle is now {state} for the shared broadcast queue "
        f"'{chat['active_queue']}'.\n"
        f"(This affects destinations that don't have their own dedicated "
        f"queue — see /setdestqueue for per-destination shuffled queues.)"
    )
...
def main():
    ...
    app.add_handler(CommandHandler("setdestqueue", set_dest_queue))
    app.add_handler(CommandHandler("toggledestshuffle", toggle_dest_shuffle))
